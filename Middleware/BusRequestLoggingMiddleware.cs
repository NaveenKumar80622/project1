...
public class BusRequestLoggingMiddleware
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        _logger.LogInformation(
            "[{CorrelationId}] [T4] Bus Response Sent:\nStatus: {StatusCode}\nBackend Processing Time: {ElapsedMs}ms\nPayload:\n{Payload}\n==================================================", 
            correlationId, context.Response.StatusCode, elapsedMs, formattedResponseBody);

        _logger.LogInformation("[{CorrelationId}] Total End-to-End Bus Time (T4 - T1): {ElapsedMs}ms", correlationId, elapsedMs);
    }
